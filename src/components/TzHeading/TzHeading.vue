...
const props = withDefaults(defineProps<TzHeadingInterface>(), {
  level: LevelEnum.h1,
  size: SizeEnum.small,
...
const sizeStyles = computed(() => useSizeStyles(props.size));
...
<template>
  <component :is="level" :class="`${baseStyles} ${sizeStyles} ${variantStyles}`">
    <slot />
  </component>
</template>
